...
void JugemMove::calc() {
    ...
    f32 fVar2 = (EGG::Mathf::abs(m_1c.y - m_pos.y) - 200.0f) / 100.0f;
    f32 fVar7 = std::clamp(fVar2, 0.0f, 1.0f);

    f32 dVar6 = 0.4f + fVar7 * (0.8f - 0.4f);
    f32 dVar5 = std::clamp(dVar6, 0.0f, 1.0f);

    EGG::Vector3f local_68 = pos - m_f4;
    local_68.y = dVar5 * (pos.y - m_f4.y);
    EGG::Vector3f vStack_bc = m_1c - local_68;
    EGG::Vector3f local_74 = vStack_bc - m_pos;
    ...
    if (!m_140) {
        ...
        m_144 = std::min(1.0f, m_144 + 0.02f);
        m_velDir = Interpolate(m_144, local_80, local_74);
        fVar7 = 25.0f;
    }
    ...
    if (m_150) {
        EGG::Vector3f local_a4 = EGG::Vector3f(local_68.x, 0.0f, local_68.z);
...
EGG::Vector3f JugemMove::calcOscillation(const EGG::Matrix34f &mat) {
    ...
    f32 sinX = EGG::Mathf::CosFIdx(RAD2FIDX * m_phaseX);
    f32 sinY = EGG::Mathf::SinFIdx(RAD2FIDX * m_phaseY);
    EGG::Vector3f v = EGG::Vector3f(AMPLITUDE_X * sinX, AMPLITUDE_Y * sinY, 0.0f);
...
EGG::Matrix34f JugemMove::FUN_807202BC() {
    ...
    m_118 = Interpolate(0.1f, m_118, m_velDir);
    f32 fVar2 = m_118.normalise();
    fVar2 = std::min(20.0f, fVar2);

    m_118 *= fVar2;
    EGG::Vector3f avStack_38 = m_118 * 1.5f;
