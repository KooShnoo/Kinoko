...
void KartMove::calcAutoDrift() {
    ...
    if (!canStartDrift() || state()->isOverZipper() || state()->isHalfpipeWall() ||
...
f32 KartMove::calcWallCollisionSpeedFactor(f32 &f1) {
    ...
    if (state()->isUNK1000() || state()->isOverZipper()) {
        return 1.0f;
    }
...
void KartMove::calcWallCollisionStart(f32 param_2) {
    ...
    if (!state()->isUNK1000() && !state()->isOverZipper() && param_2 < 0.9f) {
...
void KartMove::activateZipperBoost() {
    ...
    f32 boostDuration = state()->isZipperTrick() ? TRICK_DURATION : BASE_DURATION;
    ...
    if (state()->isWheelie()) {
        ...
        if (!state()->isAutoDrift() ||
            ...
            if (++m_autoHardStickXFrames > 15) {
                bCancelAutoWheelie = true;
            }
        }

        ++m_wheelieFrames;
        if (m_turningParams->maxWheelieFrames < m_wheelieFrames || bCancelAutoWheelie ||
