...
void KartMove::calcCannon() {
    ...
    const f32 forwardLength = forward.normalise();
    forwardXZ.y = 0;
    forwardXZ.normalise();
    ...
    // exiting cannon
    if ((forwardLength < 30.0f || local94.dot(forwardXZ) <= 0.0f) && state()->isInCannon()) {
    ...
    cannonOrientation.mulRow33(0, EGG::Vector3f::ez);
    cannonOrientation.mulRow33(1, EGG::Vector3f::ez);
    m_speed = m_baseSpeed;
    const auto *cannonPoint =
            System::CourseMap::Instance()->getCannonPoint(state()->cannonPointId());
    s16 cannonParameterIdx = cannonPoint->parameterIdx();
    if (cannonParameterIdx < 1) {
        cannonParameterIdx = 0;
    } 
    assert(static_cast<size_t>(cannonParameterIdx) < CANNON_PARAMETERS.size());
    ...
    if (cannonParams.height > 0.0f) {
        const f32 fVar9 =
                EGG::Mathf::SinFIdx((1.0f - (forwardLength / m_cannonEntryOfsLength)) * 180.0f * DEG2FIDX);
...
void KartMove::calcRotCannon(const EGG::Vector3f &forward) {
    ...
    const EGG::Quatf newRot = local80.slerpTo(localB8.multSwap(local80), 0.3f);
